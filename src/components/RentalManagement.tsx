...
export default function RentalManagement() {
  ...
  const fetchRentals = async () => {
    const res = await fetch(`${apiBaseUrl}/api/rentals/getRentals/${userId}`);
  ...
  const handleCreateRental = async (newRental: Rental) => {
    try {
      const res = await fetch(`${apiBaseUrl}/api/rentals/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({...newRental,userId}),
      });
      const data = await res.json();
      fetchRentals();
  ...
  const handleUpdateRental = async (updatedRental: Rental) => {
    try {
    await  fetch(`${apiBaseUrl}/api/rentals/updateRentals/${updatedRental.userId}/${updatedRental.rentalId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDeleteRental = (rentalId: number) => {
    try {
      fetch(`${apiBaseUrl}/api/rentals/deleteRentals/${userId}/${rentalId}`, {
        method: "DELETE",
      });
      fetchRentals();
    } catch (error) {
